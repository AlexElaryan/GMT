...
// delivery Documents plusMinus

let deliveryDocumentPlusMinus = document.querySelectorAll('.ddr_title img');
let deliveryDocumentText = document.querySelectorAll('.delivery_documents_right-item p');
let ddtisOpen = false;

deliveryDocumentPlusMinus.forEach((el,i) => {
  el.onclick = () => {
    if (!ddtisOpen) {
      deliveryDocumentText[i].style.display = 'flex';
      el.src = 'img/crossWht.svg';
      el.style.background = '#088269';
      ddtisOpen = true;
    }
    else if (ddtisOpen) {
      deliveryDocumentText[i].style.display = 'none';
      el.src = 'img/plusBlck.svg';
