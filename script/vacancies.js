...
let faqBtn = document.querySelectorAll('.faq_item-top > div');
let faqBtnImg = document.querySelectorAll('.faq_item-top > div img');
let faqBlck = document.querySelectorAll('.faq_item-block');

let isOpenFaqBlck = [];

for (let i = 0; i < faqBlck.length; i++) {
    isOpenFaqBlck.push(false);
}

faqBtn.forEach((el,i) => {
    el.onclick = () => {
        if (isOpenFaqBlck[i] === false) {
            faqBlck[i].style.display = 'flex';
            el.classList.add('faq_item-minus');
            faqBtnImg[i].src = 'img/crossWht.svg';
            isOpenFaqBlck[i] = true;
        }
        else if (isOpenFaqBlck[i] === true) {
            faqBlck[i].style.display = 'none';
            el.classList.remove('faq_item-minus');
            faqBtnImg[i].src = 'img/plusBlck.svg';
            isOpenFaqBlck[i] = false;
        }
    }
})