...
let toFavAndNoFav = document.querySelector('.favPage h1');
let favPage = document.querySelector('.favPage');
let isFav = false;

toFavAndNoFav.onclick = () => {
  if (!isFav) {
    favPage.classList.add('NoFavorites');
    isFav = true;
  }
  else if (isFav) {
    favPage.classList.remove('NoFavorites');
    isFav = false;
  }
}
...
// let specific_items = document.querySelectorAll('.specific_items');
// let swiperContainer = document.querySelector('.mySwiper3 .swiper-wrapper');
// let transformValue;

// // let specific_childs = document.querySelectorAll('.specific_items div');
// let swiperProds = document.querySelectorAll('.mySwiper3 .slide3');
// let swiperWidth = swiperProds[0].offsetWidth;
// // specific_childs.forEach(el => {
// //   el.style.width = `${swiperWidth}px`;
// // })

// let specificBlock = document.querySelector('.comp_specific-block > div:last-of-type');
// let spec_blockWidth =  swiperWidth * 5;
// specificBlock.style.width = `${spec_blockWidth}px`;
// specific_items.style.width = `${spec_blockWidth}px`;

// let catalog_text = document.querySelector('.catalog_text');
// let specLeft = document.querySelector('.comp_specific-block > div:first-of-type');
// specLeft.style.width = `${catalog_text.offsetWidth}px`;

// setInterval(() => {
//   let computedStyle = window.getComputedStyle(swiperContainer);
//   transformValue = computedStyle.getPropertyValue('transform');

//   specific_items.forEach(el => {
...
let specificBlock = document.querySelector('.comp_specific-block > div:last-of-type');
let spec_blockWidth =  swiperWidth * 6;
// specificBlock.style.width = `${spec_blockWidth}px`;

specific_items.forEach(el => {
  el.style.width = `${spec_blockWidth}px`;
});
...
let comp_specific_block = document.querySelector('.comp_specific-block');

comp_specific_block.style.gridTemplateColumns = `${catalog_text.offsetWidth}px calc(100% - ${catalog_text.offsetWidth}px)`;
